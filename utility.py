...
def get_position(env, field_size):
    ...
    virtual_position = np.mean(swarm_position, axis=0) + [50., 0.]
...
def path_prediction(swarm_position, arcxPre, arcyPre, killing_intensity, swarmIndAvoiding, field_env, N):
    ...
    ind1_temp = np.round(swarm_position[:, 0] - 1)
    ind2_temp = np.round(swarm_position[:, 1] - 1)
    ind1_temp[ind1_temp > field_size - 1] = field_size - 1
    ind2_temp[ind2_temp > field_size - 1] = field_size - 1
    swarm_intensity = field_env[ind1_temp.astype(int), ind2_temp.astype(int)]
